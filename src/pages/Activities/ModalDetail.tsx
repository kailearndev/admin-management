...
const ModalEdit: React.FC<ModalEditProps> = ({ isOpen, onClose, id }) => {
    useEffect(() => {
        if (id) {
            handleGet()
        }
    ...
    const handleFileImageHeader = async (e: ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            let res: any = await UserService.uploadFile(fileUpload as File);
            if (res?.statusCode === 200) {
                const newImageHeader = {
                    fileId: res?.data,
                    url: '',
                    fileName: fileUpload.name,
                    contentType: fileUpload.type,
                };
                formik.setFieldValue('imageHeaders', [newImageHeader]);
                toast.success('Upload success !!');
            }
        }
    };
    const handleUpdate = async (values: ActivitiesValueUpdate) => {
        let res = await ActivitiesService.updateActivities(id, {
            id: id,
            imageHeaders: values.imageHeaders.flatMap(item => item.fileId),
            title: values.title,
    ...
    console.log(formik.values.imageHeaders);

    return (
        <>

            {isOpen && (
                <Modal open={isOpen} backdrop={false} className='w-11/12 max-w-5xl'>
                        ...
                        <div className='grid xl:grid-cols-2 gap-4' >
                            ...
                            < div className='flex flex-col gap-3' >
                                ...
                                < input name='imageArticleUrl'
                                    value={formik.values.imageHeaders} type="text"
